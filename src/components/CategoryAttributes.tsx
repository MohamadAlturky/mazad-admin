...
const CategoryAttributes: React.FC = () => {
    ...
    const fetchAttributes = async () => {
        ...
        setIsLoading(true);
        try {
            const { data } = await axios.get(`http://localhost:5032/api/categories/attributes/${categoryId}`, {
                headers: {
    ...
    const toggleAttribute = async (attributeId: number) => {
        try {
            const { data } = await axios.put(
                `http://localhost:5032/api/categories/attributes/${categoryId}/${attributeId}`,
                {},
                {
                    headers: {
                        'Accept-Language': language,
            ...
            if (data.success) {
                // Refresh the attributes list after successful toggle
                fetchAttributes();
            }
            else
            {
        ...
        <AdminLayout loading={isLoading}>
            <Card className="border-purple-200">
                <CardHeader className="flex flex-row items-center justify-between" >
                    <CardTitle className="text-purple-900">{t('categoryAttributes')}</CardTitle>
                    <div className="relative mt-4 max-w-md">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                className={cn(
                                    "pl-10 pr-10 bg-white border-purple-200 focus:border-purple-500 focus:ring-purple-500",
                        ...
                        {searchQuery && (
                            <p className="text-sm text-gray-500 mt-2">
                                {filteredAttributes.length} {t('resultsFound')}
                            </p>
                        )}
                    </div>
                </CardHeader>
                ...
                <CardContent>
                    {filteredAttributes.length === 0 ? (
                        <div className="flex flex-col justify-center items-center h-32 text-purple-600">
                            ...
                            {searchQuery ? t('noMatchingAttributes') : t('noDataAvailable')}
                        </div>
                    ) : (
                            ...
                            {filteredAttributes.map((attribute) => (
                                ...
                                >
                                    <div className="flex flex-col gap-1">
                                        <span className="font-medium text-purple-900">{attribute.name}</span>
                                        <div className="flex gap-2">
                                            <Badge
                                                variant="outline"
                                                className="bg-purple-50 text-purple-800 border-purple-200"
                                            >
                                                {attribute.attributeValueTypeString}
                                            </Badge>
                                            <Badge
                                                variant={attribute.isActive ? 'default' : 'secondary'}
                                                className={attribute.isActive ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'}
                                            >
                                                {t(attribute.isActive ? 'active' : 'inactive')}
                                            </Badge>
                                        </div>
                                    </div>
